...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        return (V, A) if self.output_attention else (V, None)
...
class ProbAttention(nn.Module):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):
        ...
        index_sample = torch.randint(L_K, (L_Q, sample_k)).to(K.device)
        K_sample = K_expand[:, :, torch.arange(L_Q).unsqueeze(1), index_sample, :]
        Q_K_sample = torch.matmul(Q.unsqueeze(-2), K_sample.transpose(-2, -1)).squeeze(-2)

        M = Q_K_sample.max(-1)[0] - Q_K_sample.mean(-1)
        n_top_safe = min(n_top, L_Q)
        M_top = M.topk(n_top_safe, sorted=False)[1]

        batch_idx = torch.arange(B)[:, None, None].to(Q.device)
        head_idx = torch.arange(H)[None, :, None].to(Q.device)

        Q_reduce = Q[batch_idx, head_idx, M_top, :]
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        queries = self.query_projection(queries).view(B, L_Q, H, D_k).transpose(1, 2)
        keys = self.key_projection(keys).view(B, L_K, H, D_k).transpose(1, 2)
        values = self.value_projection(values).view(B, L_K, H, D_v).transpose(1, 2)
